...
SMO（Sliding Mode Observer）是一种状态观测器，通过观测反电动势来估算转子位置。

**电机数学模型（α-β坐标系）**：

```
dIα/dt = (1/Ls)(Vα - Rs×Iα + ω×Ls×Iβ - Eα)
dIβ/dt = (1/Ls)(Vβ - Rs×Iβ - ω×Ls×Iα - Eβ)
```
...
| 概念 | 说明 |
|------|------|
| **滑模控制** | 一种鲁棒控制方法，通过不连续控制使系统状态到达滑模面 |
| **反电动势** | 电机旋转时线圈切割磁感线产生的电压，与转速成正比 |
| **反正切查表** | 使用查找表实现快速反正切计算，避免浮点运算 |
| **IF启动法** | 注入直轴电流使电机锁定，然后逐步加速到SMO可工作范围 |
| **Clarke变换** | 将三相量转换为α-β坐标系 |
